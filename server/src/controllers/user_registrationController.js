...
const vetModel = require('../models/vetsModel');
...
module.exports.register = async (req, res) => {
  ...
  try {
    ...
    {
        const user = await vetModel.findOne({ email: req.body.email });
        if (user) {
            // If email exists, send error response
            return res.send({ code: 400, message: 'Email already registered' });
        }
...
module.exports.login = (req, res) => {
    ...
    if(req.body.userType == "petowner"){
        petOwnerModel.findOne({ email: req.body.email })
        .then(user => {
            ...
            if (!user) {
                ...
                bcrypt.compare(req.body.password, user.password, (err, result) => {
                    if (err) {
                        ...
                        // });
                        res.cookie('cookieName',1010, { maxAge: 900000, httpOnly: true });
                        res.send({
                            userDetails: user,
                            email: user.email,
                            userType: req.body.userType,
    ...
    {
        vetModel.findOne({ email: req.body.email })
        .then(user => {
            // Find the user by email
            if (!user) {
                // If user is not found, send error response
...
module.exports.sendotp = async (req, res) => {
    ...
    if (info.messageId) {
        ...
        console.log(info, 84)
        petOwnerModel.updateOne({ email: req.body.email }, { otp: _otp })
            .then(result => {
                res.send({ 
                     userType: req.body.userType,
                    code: 200, 
        ...
        let user = await vetModel.findOne({ email: req.body.email })
    // send to user mail
    if (!user) {
        res.send({ code: 500, message: 'user not found' })
    }
    ...
    if (info.messageId) {
        ...
        console.log(info, 84)
        vetModel.updateOne({ email: req.body.email }, { otp: _otp })
            .then(result => {
...
module.exports.submitotp = (req, res) => {
    console.log(req.body);
    if(req.body.userType == 'petowner'){
        petOwnerModel.findOne({ otp: req.body.otp }).then(result => {
            bcrypt.hash(req.body.password, 10, (err, hash) => {
                if (err) {
                    res.send({ code: 500, message: 'Server err' });
                } else {
                    petOwnerModel.updateOne({ email: result.email }, { password: hash })
                        .then(result => {
        ...
        vetModel.findOne({ otp: req.body.otp }).then(result => {
            bcrypt.hash(req.body.password, 10, (err, hash) => {
                if (err) {
                    res.send({ code: 500, message: 'Server err' });
                } else {
                    vetModel.updateOne({ email: result.email }, { password: hash })
                        .then(result => {
                            res.send({ code: 200, message: 'Password updated' })
                        })
                        .catch(err => {
